...
from core.data_utils import process_viewpoint_label, TransLightning, resize_pad, random_crop
...
def angle_to_sincos(label_deg):
    az, el, inplane = label_deg
    az = deg2rad(az)
    el = deg2rad(el)
    inplane = deg2rad(inplane)
    return np.array([
        np.cos(az), np.sin(az),
        np.cos(el), np.sin(el),
        np.cos(inplane), np.sin(inplane)
    ], dtype=np.float32)
...
class Pascal3D(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = angle_to_sincos(label)
        label = torch.from_numpy(label).float()

        return [im, im_pos], label
